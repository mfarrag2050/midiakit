...
## مكتبات SVG (2026-09-04)

طبقة SVG في `packages/engine/src/layers/svg.ts` — تحلّل SVG إلى أشكال
أوّلية وترسمها على Canvas 2D عبر Path2D. **لا محرّك رسم ثانٍ** (احترام
ADR-001 والقاعدة 2 في CLAUDE.md).

### svg-parser (Rich-Harris) — v2.1.0

- **الاستعمال:** يحلّل نصّ SVG إلى AST بسيط (شجرة عناصر بخصائصها).
- **الترخيص:** MIT — https://github.com/Rich-Harris/svg-parser (معلن
  في `package.json`؛ لا LICENSE منفصل — نص MIT يُرفَق عند الشحن).
- **التبعيات:** 0.
- **التفرّد:** أعلى تنزيل أسبوعي (17M) لمحلّل SVG→AST خالص لا يرسم.

### svg-pathdata (nfroidure) — v9.0.0

- **الاستعمال:** يفكّك سلسلة `d="..."` إلى أوامر (M/L/C/Q/A…) بأنواع
  TS — أساس تحويل الأوامر إلى Path2D.
- **الترخيص:** MIT — https://github.com/nfroidure/svg-pathdata
- **التبعيات:** 0. **الصيانة:** 2026-03 (نشط).

### svg-arc-to-cubic-bezier (colinmeinke) — v3.2.0

- **الاستعمال:** يفكّك أمر القوس `A` إلى Bézier مكعّبات. Canvas Path2D
  لا يدعم `A` بشكل مباشر — هذا الجسر إجباري لتغطية القسم.
- **الترخيص:** ISC — https://github.com/colinmeinke/svg-arc-to-cubic-bezier
- **التبعيات:** 0.

**الرفضة الصريحة:** `@resvg/resvg-js` — يحمل محرّك Rust native داخلياً
