...
| `cdp-s23.mjs` | S23 مساحة العمل — workspace شاشة واحدة · ٤ مقاسات · لوحة تشكيل · `_word_` · content.locale · persistence — على mk-api الحقيقي | `s23-workspace-single-screen.png` · `s23-size-*.png` (٤) · `s23-tashkeel-kbd.png` · `s23-word-accent.png` · `s23-diacritize-flow.png` · `s23-locale-latin.png` · `s23-saved.png` · `s23-reopen-preserved.png` |
| `cdp-reels.mjs` | reels/454→456→458 — محرّر الخطّ الزمني على `/reels` (mock-free، وتوكنٌّ محليٌّ لدخول AppShell — 473): عرض · تحديد · سحب ماوس فعليّ عبر أحداث CDP (+٥ ث) · شطر عند رأس القراءة باختصار S · أرقام عربية-هندية · زوم وتمرير (⌘+عجلة وأزرار ولوحة مفاتيح) مع إثبات هندسيّ لعرض قطعة عند مستويَي زوم — وتحقّق هندسة DOM بعد كلّ خطوة | `claude/reports/458-shots/` (أو argv[2]): `reels-01-idle.png` · `reels-02-selected.png` · `reels-03-after-drag.png` · `reels-04-after-split.png` · `reels-05-arabic-digits.png` · `reels-06-zoomed.png` |
| `cdp-reels-474.mjs` | reels/474 §٣–§٤ — موضعٌ لا يتصادم: قطعتان نصّيّتان متداخلتان زمنيّاً عبرَ زرِّ الإضافةِ في مسارَين ← البحثُ يضعُ كلَّ واحدةٍ بلا تحذيرٍ جديد (خطّةٌ بالـctx نفسِه) ← تصادماتٌ صفرٌ ولا شريطَ تنبيه، والفصلُ الرأسيُّ مثبتٌ هندسيّاً (yGap) — والحمراءُ الشاهدةُ (addItem الخام) في vitest | `claude/reports/474-shots/` (أو argv[2]): `reels-15-no-collision.png` |
| `cdp-reels-475.mjs` | reels/475 §٢–§٤ — الوسمُ الصادق: الوسمُ مُصيَّرٌ مرئيّاً (offsetParent) قربَ العنوانِ ونصُّهُ من القاموس، مع شاهدٍ سلبيٍّ يثبتُ أنّ الفحصَ يفحصُ الرؤيةَ لا الوجود | `claude/reports/475-shots/` (أو argv[2]): `reels-16-honest-notice.png` |
