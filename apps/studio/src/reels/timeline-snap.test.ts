...
describe('moveItemSafe — يقصّ عند الجار ولا يتراكب', () => {
  ...
  it('يقف عند جاره لا عند الصفر', () => {
    // b = [10,16] طولُها 6، وa = [0,8]. الإزاحةُ -20 تبلغ الصفرَ نظريّاً،
    // لكنّ الجارَ أسبقُ من الحافّة: تقف عند 8 لا عند 0.
    const r = it_(moveItemSafe(BASE, 'tr_media', 'b', -20), 'tr_media', 'b');
    expect([r.start, r.end]).toEqual([8, 14]);
  });

  it('يقصّ عند الصفر حين لا جارَ قبله', () => {
    const r = it_(moveItemSafe(BASE, 'tr_media', 'a', -20), 'tr_media', 'a');
    expect([r.start, r.end]).toEqual([0, 8]);
  });

  it('لا يُنتج تراكباً مهما كان الإزاحة', () => {
    for (const id of ['a', 'b', 'c']) {
      for (const d of [-100, -7, -0.1, 0.1, 7, 100]) {
        const r = moveItemSafe(BASE, 'tr_media', id, d);
        const it = it_(r, 'tr_media', id);
        expect(wouldOverlap(r, 'tr_media', id, it.start, it.end)).toBe(false);
      }
    }
  });
