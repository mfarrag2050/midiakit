...
function launch(target) {
  if (!['api', 'worker'].includes(target) || process.argv.length !== 3) {
    throw new DevLaunchError('DEV_LAUNCH_USAGE: node apps/api/scripts/dev-launch.mjs api|worker');
  }
  assertInheritedEnvironment(process.env);
  const environment = cleanDevEnvironment(process.env, readLocalEnvironment());
  const entry = target === 'api' ? 'src/server.ts' : '../renderer/src/api-worker.ts';
