...
- **A18.5. ربط العامل بالتخزين** (2026-09-06 · قبل أول عميل) —
  ...
  **موضعها بين A18 و A19 (لا في نهاية القائمة):** قبل أوّل عميل،
  لا بعد A25. بلا هذا الربط لا مسار منتج قابل للاستخدام.
...
### 3.3.α ترتيب المراحل بعد A20 (محسوم 2026-09-06)

**الترتيب المعتمد:**

```
A20 → A26 → A27 → A21 → A22 → A23 → A24 → A25 → A28
```

**السبب:** A21 (الاشتراكات) تقرأ حدود الباقات، و A26 (طبقة
الإعداد · docs/17 §3.5) يبني جدول `plans` الذي يحملها. فبناء
A21 قبل A26 يعني فرض حدّ من ثابت في الكود ثم إعادة كتابته
لاحقاً. الترتيب أعلاه يُقدِّم البنية على المستهلك.

**لا إعادة ترقيم:** الأرقام تبقى كما وُثِّقت (A21..A28 بالترتيب
الرقمي في التعريف)، والترتيب التنفيذي هذا يُعلَن هنا. رسائل
الالتزام تستعمل الرقم لا الموضع (L-57).
