...
### 4.14 مساحة عمل موحّدة — لا تعتمد على A جديد (2026-09-08)

**السياق:** المالك فتح الاستوديو فلم يجد أين يبني. الحقول في صفحة،
المعاينة في أخرى، التصدير في ثالثة، المقاس والصيغة مبرمَجان. §S23
يجمعها ويُخرج القيم المبرمجة إلى اختيار.

**قاعدة الاعتماد:** يعتمد على S12 (محرّر المشروع) + S13 (المعاينة
الحيّة) — كلاهما مبنيّ. **لا يعتمد على A جديد** — واجهة حرّة يستهلك
`GET/PATCH /v1/projects/:id` القائم و `renderFrame` من المحرّك.

- **S23.** مساحة عمل المشروع (Project Workspace) — شاشة واحدة تجمع:
  - **حقول المحتوى** المشتقّة من `template.definition.fields`.
  - **المعاينة الحيّة** بأبعاد المخرَج المختار (`SIZE_OPTIONS` من
    `docs/09 §المخرجات`: بطاقة X 1080×1080 · بطاقة إنستغرام 1080×1350
    · ستوري/ريلز 1080×1920 · فيديو 1080×1920). المعاينة تتبع المقاس
    فوراً — **«المعاينة = المخرَج»** شرط محفوظ.
  - **مبدّل الصيغة** (png/mp4) مقيَّد بالمقاس: X + Instagram = png
    فقط · Story/Reel = الاثنين · Video = mp4 فقط.
  - **مبدّل `content.locale`** (ar/latin) منفصل عن `LocaleSwitcher`
    (لغة الموظف). يقود `applyLocaleToBrand(brand, content.locale)`
    قبل `renderFrame` — كسر السلوك في `docs/04 §L-49`.
  - **تدفّق التشكيل الست خطوات** من `docs/09 §«التشكيل — العميل
    يملك القرار»`: عارٍ ⇒ «شكّل» ⇒ حقل قابل للتحرير ⇒ تصحيح ⇒
    المعاينة تتبع ⇒ حفظ. النموذج اقتراح لا سلطة. **لا حقل ظلّ.**
  - **لوحة الحركات المساعِدة** (٨ أزرار: فتحة/ضمة/كسرة/سكون/شدة/
    تنوين ×3) تُدرج عند مؤشّر الحقل.
  - **«أزل التشكيل»** — regex واحد يجرّد كل العلامات (U+064B–U+0650،
    U+0651–U+0652، U+0670، U+0640).
  - **`_word_` accent span** — زرّ يلفّ التحديد بشرطتَين سفليّتَين
    (`docs/04`).
  - **حفظ التشكيل مع المشروع** — يُخزَّن في `content.headline` (لا حقل
    منفصل). `reopen` يعرض ما حُفظ حرفياً — لا إعادة تشكيل تُضيع
    تحرير العميل.

**بوابات S23 (١٠ بوابات · G-S23-1..10):** موثَّقة في تقرير التسليم
(`PHASES-studio.md §S23`) — أهمّها: G-S23-3 شاشة واحدة · G-S23-4
أربع لقطات مقاس تُظهر أبعاد المعاينة تتغيّر · G-S23-7 persistence
بعد reload · G-S23-8 latin ⇒ preview LTR · G-S23-9 `_word_` من
الواجهة.

**تكامل التشكيل — proxy لا اتّصال مباشر:** `apps/studio/app/api/
diacritize/route.ts` هو Next server-route يعمل proxy إلى
`services/diacritizer:19080`. **السبب:** الخدمة بلا CORS (مذكور
صراحة في `services/diacritizer/diacritizer_service/main.py`)، فالمتصفح
لا يستطيع الاتّصال المباشر. Proxy خادم-جانب في Next يحلّ المشكلة بلا
لمس `services/**`.

**السلوك حين الخدمة مُطفأة:** ECONNREFUSED ⇒ `SERVICE_UNAVAILABLE`
⇒ لوحة صفراء «الخدمة غير متاحة» + زرّ «شكّل» مفعَّل (فشل عابر لا
دائم).

### 4.15 الخدمات المعزولة — جرد ومسؤوليّة (2026-09-08)

**تصحيح تصنيف سابق:** في تخطيط 2026-09-07 صُنّفت `services/diacritizer`
كـ«غير مبنيّة» لأنّها لا تعمل وقت الجرد. **هذا خطأ تصنيف:** الخدمة
مبنيّة (Python module + FastAPI + النموذج مثبَّت عبر pip) — ما ينقص
واجهتها الاستوديويّة. **الخدمات مبنيّة بدرجات، وما ينقص واجهاتها في
الاستوديو.**

**الجرد الفعلي (تحقّق git · 2026-09-08):**

| الخدمة | مسار | endpoint | منفذ | backend | واجهة studio |
|---|---|---|---|---|---|
| diacritizer | `services/diacritizer/` | `POST /diacritize` · `GET /health` | 19080 | `arabic-diacritizer` (pip · MIT · النموذج جاهز) | **مبنيّة (S23 · 090e0e6):** `/api/diacritize` proxy + زرّ «شكّل» + لوحة الحركات + «أزل التشكيل» في محرّر المشروع |
| face-detector | `services/face-detector/` | `POST /detect` · `GET /health` | 19082 | `mtcnn` + `PIL` (pip) | **غير موجودة.** الاستهلاك المتوقّع: `packages/engine/src/layers/smart-crop.ts` وقت الرندر (خادم-جانب) — أو زرّ «اكشف الوجوه» على asset (قرار مؤجَّل) |
| transcriber | `services/transcriber/` | `POST /transcribe` · `GET /health` | 19081 | `faster-whisper` (pip · MIT) | **غير موجودة.** مطلوبة لتحرير كابشن الفيديو (`docs/07 §caption`) — تُبنى في ticket لاحق |

**تشغيل الخدمات (يدوي لكلّ):**
- الإعداد الأوّل: `cd services/<name> && python3.12 -m venv .venv &&
  source .venv/bin/activate && pip install -e .` (راجع README الخدمة).
- التشغيل: `source .venv/bin/activate && uvicorn <module>.<app>:app
  --host 127.0.0.1 --port <19080|19081|19082>`.

**قاعدة الواجهة:** كل خدمة تحتاج واجهة studio جديدة ⇒ **Next server
route (`apps/studio/app/api/<name>/route.ts`)** يعمل proxy، لا اتّصال
مباشر من المتصفح. السبب المُوثَّق: الخدمات بلا CORS، والمتصفح يرفض
cross-origin. Proxy = تجنّب تعديل الخدمة + حفظ واجهة نظيفة.

**قاعدة الفشل:** ECONNREFUSED / timeout ⇒ رمز `SERVICE_UNAVAILABLE`
(clientOnlyCode) + لوحة صفراء «الخدمة غير متاحة» + زرّ الفعل مفعَّل
للمحاولة. **لا Alert أحمر** — الخدمة غير مُشغَّلة ≠ خطأ نظام.
...
| S23 | S12 + S13 (لا A جديد) | `PATCH /v1/projects/:id` (content) · `services/diacritizer` (اختياري — proxy عبر Next) |
