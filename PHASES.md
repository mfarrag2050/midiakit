...
| **3.9** | **حراسة المحرك — البوابات الست** (G1..G6) | **أسبوع** | كل بوابة موصوفة في PHASES §امتصاص docs/marketing/أ لديها اختبار دائم يفشل البناء عند انحدارها | ☑ |
...
| # | البند | الحالة | ملاحظة |
|---|---|---|---|
| A1 | G1 نظافة الأصول الأناضولية — `scripts/verify-clean-of-aa.mjs` | ☑ (2026-09-04) | **مُنجَز في الدفعة الأولى** — سكربت الرموز الستة + canary سلبي. مكمِّل لـcheck-no-brand-leak (رموز كود مقابل نصوص عيّنة) |
| A2 | G2 عزل الهوية والقالب — `scripts/verify-brand-template-isolation.mjs` | ☑ (2026-09-04) | **مُنجَز في الدفعة الأولى** — snapshot 24/24 + canary يعدّل layers/logo.ts |
...
# المرحلة 3.9 — البوابات الست (مُغلَقة 2026-09-04)

**الحالة:** ست بوابات محروسة، كل واحدة **بثلاثة اختبارات على الأقل**:
وجود (يفشل عند حذف الميزة) · ثبات (يفشل عند اهتزازها) · **سلبي صريح**
(يفشل عند خللٍ متعمَّد يمثّل انحدار مستقبلي).

| # | البوابة | السكربت | الحارس | السلبي المُثبَت | تاريخ |
|---|---|---|---|---|---|
| G1 | نظافة الرموز الستة الموروثة (`AA_LOGO`, `CV_LOGO`, `HelveticaNeueLTArabic`, `HNArabic`, `AA-60-SANIYE`, `aa_customLogo`) | `scripts/verify-clean-of-aa.mjs` | وجود | canary سلبي — إدخال رمز واحد يفشل | 2026-09-04 |
| G2 | عزل الهوية والقالب عن المحرك | `pnpm verify:snapshot` + canary | وجود + ثبات | تعديل `layers/logo.ts` يُغيّر لقطات ⇒ يفشل | 2026-09-04 |
...
| G6 | عزل المستأجرين (RLS) | `scripts/verify-tenant-isolation.mjs` | وجود + ثبات | `RLS_MOCK_LEAK=1` يحاكي جدولاً بلا RLS ⇒ يفشل. **معلَّق بشرط `PF_MEDIAKIT_DATABASE_URL`** — يُفعَّل تلقائياً عند أوّل هجرة RLS في المرحلة 4 | 2026-09-04 (skeleton) |
