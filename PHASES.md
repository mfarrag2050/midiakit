...
- ☐ **مسح الميزات المنهجي** — مجدول **بعد الصوت وقبل المرحلة 4**.
      مسح docs/07 كان عن السوق والتموضع لا الميزات؛ بندان أساسيان
      (الإسناد والموشن جرافيك) اكتُشفا بالصدفة من ذاكرة المالك في غرفة
      الأخبار لا من مسح. المسح الأول قال «لماذا يشترون»؛ الثاني يقول
      «ماذا يستخدمون» — L-26.
      - نطاق: ما يفعله Placid · Bannerbear · Creatomate · Canva تحديداً
      - ما يطلبه مستخدموهم في منتدياتهم ولا يجدونه
      - ما تفعله غرف الأخبار العربية يدوياً في CapCut/Premiere اليوم
      - **الهدف:** جمع كل النواقص دفعة واحدة قبل بناء المنصة — إضافة
        ميزة تمسّ نموذج البيانات بعد المنصة أغلى بأضعاف.

- ☐ **طبقة الإسناد (attribution) — متطلب تحريري وقانوني** — البند
      الحالي `content.source` نصّ حرّ لا يكفي لإعادة النشر عن السوشيال
      (وهو نمط الاستهلاك السائد في غرف الأخبار). المطلوب طبقة قالب
      مستقلة:
      ```
      { platform: 'tiktok'|'x'|'instagram'|'youtube'|'telegram'|'facebook',
        mode: 'handle'|'name'|'both',
        handle: '@username', name: 'اسم المصدر',
        anchor: 'bottom-left', showPlatformLogo: true }
      ```
      **تحذير قانوني:** شعارات المنصات علامات تجارية. الاستخدام الاسمي
      للإسناد مسموح عادة، لكن لكل منصة إرشادات علامة تجارية خاصة —
      يُتحقق منها لكل شعار قبل التنفيذ ويُوثَّق في `ATTRIBUTIONS.md`.
      **الأولوية:** متطلب تحريري وقانوني، لا تجميلي — الوكالات تضيفه
      يدوياً في CapCut اليوم، وهذا بالضبط العمل المتكرر الذي تبرّره
      أداتنا.

- ☐ **الموشن جرافيك المتجهي (Lottie / Skottie)** — غائب كلياً. المحرك
      يرسم نقطياً ونصوصاً فقط. الوكالات تنتج موشن جرافيك من إليستريتور →
      أفتر إفكتس → Bodymovin → JSON. **Skottie** (عارض Lottie مبني على
      Skia) وارد كتوافق معماري: `skia-canvas` عندنا يحمل عارض Lottie
      محتمل بلا تخطيط سابق.
      - **المسار (أ) الموصى به — أيام لا أسابيع:** طبقة `lottie` كنوع
        TrackItem تستدعي Skottie. تحقّق أوّلاً من إتاحته في `skia-canvas`
        تحديداً (قد يحتاج بناءً مخصصاً). البديل: `lottie-nodejs` مع
        `node-canvas`. تكامل طبيعي: عنصر lottie = TrackItem بخط زمني.
      - **المسار (ب) لاحقاً — أسبوعان+:** تحليل شجرة SVG بمحرّكنا،
        تحويل `<path>` إلى Path2D، تدرّجات وأقنعة يدوياً. أغلى لكن
        يبقي القيمة داخل المنصة (نحن المحرّك لا المشغّل).
      - **الفرق:** (أ) يجعلنا مشغّلاً — الحركة تحدث في أفتر إفكتس
        خارج منصتنا. (ب) يبقي كل شيء داخلي لكنه استثمار طويل.
      - **تصحيح على المرجع:** `ai2html` (نيويورك تايمز) ليس الأداة —
        غرضه الرسوم الثابتة المتجاوبة والميسَّرة، لا الحركة. المعيار
        الفعلي للحركة المتجهية في الصناعة هو Lottie/Bodymovin.
